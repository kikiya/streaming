...
class ChirperClientSimulator {
    ...
    private void run() throws IOException {
        ...
        */
        ...
        /**
         * todo student homework: read up on the Sink.foreach method
         * the important thing to know now is that for each element that
         * "lands" in this sink, we call sysout println.
         */
    ...
    /**
     * This helper method is for creating an HTTP GET request.
     * @param uri
     * @return
     */
    ...
    /**
     * TODO: Create a helper for POST messages.
     * This method should be similar to the GET request but with some additional
     * information. You'll see an additional parameter "entity" ...
     * imagine how that entity object should be used to build
     * this request. Hint, there's a function in the http request
     * builder that handles entity data.
     *
     * @param uri
     * @param entity
     * @return
    ...
    private CompletionStage<HttpResponse> post(String uri, Object entity) {
        final HttpRequest request = HttpRequest.create()
                ...
                .withUri(uri);
        //todo the entity should be used... hint there's Entity.toJson(entity)
        return Http.get(actorSystem).singleRequest(request);
    }

    /**
     * The first client function we'll use is adding a user.
     * Remember the 'post' helper method? we should use this to
     * post the user data when adding a user.
     *
     * todo hint - the add user uri is http://localhost:9000/api/users.
     *
     * @param user
     */
    private void addUser(Entity.User user) {
        final CompletionStage<HttpResponse> httpResponse = null;
    ...
    /**
     * Add user friend follows the similar concepts as add user.
     *
     * @param user - the user who wants a friend to be added.
     * @param friendId - the friend you are adding to the user.
     */
    ...
    /**
     * Get user uses some other interesting pieces of akka streams, i.e. the 'Graph'
     * This graph is actually a sink that is used for printing.
     *
     * @param userId
     * @param userSink
     */
    private void getUser(String userId, Graph<SinkShape<Entity.User>, ?> userSink) {
        ...
        //todo can you imagine another flow from User to something else?
